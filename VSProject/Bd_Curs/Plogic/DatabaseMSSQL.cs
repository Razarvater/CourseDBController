...
namespace Bd_Curs
{
    public class DatabaseMSSQL : Database
    {
        ...
        public override void SetShow(MessageShow message) =>
            Show += message;
        public DatabaseMSSQL(string server,string database,string FilePath)
        {
            if (FilePath != string.Empty)
                connection = new SqlConnection($"Data Source=.\\SQLEXPRESS;AttachDbFilename={FilePath};Integrated Security=True;Connect Timeout=30;User Instance=True");//Создание объекта SQl connection
            else
                connection = new SqlConnection($"Server={server};Database={database};Trusted_Connection=True;");//Создание объекта SQl connection
        }
            
        public override async Task startConnectionAsync(string name, string password)//Подключится к базе данных 
        {
        ...
        public override async Task AuthAsync(string name, string password)//Авторизация в базе данных
        {
            try
            { 
        ...
        public override void CloseConnection()//Закрытие подключения
        {
            try
            {
        ...
        public override void GetSelectedTable(string table)
        {
            try
            {
        ...
        public override void SetQuery(string Query, List<SqlParameterStr> parameters)//Выполнение запроса
        {
            try
            { 
        ...
        public override void DropTable(string TableName)//Удаление таблицы из БД
        {
            SqlCommand command = new SqlCommand($"DROP TABLE {TableName}", connection);
            command.ExecuteNonQuery();
        } 
        public override int GetAutoIndex(string NameTable)//Получение следующего автоинкремента у таблицы
        {
